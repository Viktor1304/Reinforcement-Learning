...
def generate_episode(policy):
    """Generates a single episode using epsilon-soft policy."""
    # Setup hands to match the exploring start
    player_hand = [BlackjackGame.draw_card(), BlackjackGame.draw_card()]
    dealer_hand = [BlackjackGame.draw_card(), BlackjackGame.draw_card()]
    ...
    for i in range(1, num_episodes + 1):
        ...
        for state, action in episode:
            if (state, action) not in visited_state_actions:
                ...
                A_max = "hit"
                reward_max = 0.0
                if Q[state]["hit"] > reward_max:
                    reward_max = Q[state]["hit"]
                    A_max = "hit"
                if Q[state]["stick"] > reward_max:
                    reward_max = Q[state]["stick"]
                    A_max = "stick"
...
def main():
    print("Starting Monte Carlo epsilon-greedy for Blackjack...")
